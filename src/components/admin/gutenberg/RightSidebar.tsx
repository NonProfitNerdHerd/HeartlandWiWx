...
import type { Block, PageMeta } from '../../../types/blocks';
...
export default function RightSidebar({
	...
	const updateMeta = (patch: Partial<PageMeta>) => onMetaChange({ ...meta, ...patch });
	...
	return (
		<aside className="gb-right-sidebar">
			<div className="gb-right-header">
				<h2>{selectedBlock ? (def?.label ?? selectedBlock.type) : 'Page'}</h2>
				{selectedBlock ? <span className="gb-right-sub">Block settings</span> : <span className="gb-right-sub">Page settings</span>}
			</div>
			<div className="gb-right-content">
				{selectedBlock ? (
					...
					<div className="gb-settings-fields">
						...
						<label>Template
							<select value={meta.template} onChange={(e) => updateMeta({ template: e.target.value })}>
